...
library(randomcoloR)
set.seed(20)
names_these <- unique(data_reclustered$SingleR.pruned)
...
ggsave(file.path(fig_folder, "barplot_main_labels_RECLUSTERED.png"),
...
### UMAP of cluster names (FIGURE 1)
```{R}
set.seed(14)
to_color <- unique(data_reclustered$fctr_celltype)
palette <- distinctColorPalette(length(to_color)) |>
  setNames(to_color)
...
in_umap_shuff |>
  ggplot2::ggplot(aes(x = umap_1, y = umap_2, colour = fctr_celltype)) +
  ggplot2::geom_point(alpha = 0.4, size = 0.4) +
  ggplot2::labs(x = "UMAP Axis 1", y = "UMAP Axis 2",
                title = "UMAP Plot",
