...
sig_names_fixed <- sig_names |>
  stringr::str_replace_all("_", " ") |>
  stringr::str_remove_all("HP ") |>
  stringr::str_to_sentence()

calc_sigs <- sig_names |>
  plyr::alply(1, make_sig_list) |>
  set_names(sig_names_fixed)

sigs_in <- unique(unlist(calc_sigs))
new_counts <- data_reclustered@assays$integrated$scale.data
```

# Subsample to include only 5000 genes in addition to those in any signatures of interest

This allows for efficient processing with minimal effect on results
```{R}
set.seed(5000)
...
## Create renamed treatment groups function

```{R}
rename_trt <- function(this_treatment) {
  if (this_treatment == "control") return("Control")
  if (this_treatment == "Ptreatment") return("PTX")
  if (this_treatment == "Dtreatment") return("DEX")
  if (this_treatment == "PDtreatment") return("PTX + DEX")
}

dot_names <- stringr::str_replace_all(names(calc_sigs), " ", ".")
all_sigs <- paste(c("PLAGE"), dot_names,
                  sep = "_") |>
  append(paste(c("ssGSEA"), dot_names,
                  sep = "_"))
...
combinations <- list(c("control", "Ptreatment"),
                     c("control", "PDtreatment"),
                     c("control", "Dtreatment"))
...
for (combo in combinations) {
  ...
  p <- compareBoxplots(
    ...
    labs(x = "Calcification gene signatures",
         title = "Bootstrapped signature AUC scores",
         subtitle = paste(rename_trt(treatment1), "vs.", rename_trt(treatment2))) +
    theme(
  ...
  ggsave(file.path(fig_folder, filename), units = "in", width = 5, height = 5)
}
...
# Evaluating gene sets for publication
